...
from curriculum.experiments.her_free_maze.her_sample import her_sampler
from curriculum.experiments.her_free_maze.replay_buffer import replay_buffer
from curriculum.experiments.her_free_maze.normalizer import normalizer
import timeit
import csv
import time
...
def generate_goals(goals_l):
    ...
    inside_box = bounding_box(goals_l, min_x=-5, max_x=5, min_y=-5, max_y=5)
        
    points_inside_box = goals_l[inside_box]
        ...
        for epoch in range(self.n_epochs):
            ...
            for cycl in range(self.cycles):
                ...
                for ep in range(self.episode_number):
                    ...
                    #sample goal randomly from goal space, in free maze every goal is achievable
                    goals_g = np.random.uniform(np.array(self.goal_center) - np.array(self.goal_range), 
                                              np.array(self.goal_center) + np.array(self.goal_range), size=(30000, self.goal_size))
                    
                    import sys
                    np.set_printoptions(threshold=sys.maxsize)
                    goals = generate_goals(goals_g)
                    number_of_goals = goals.shape[0]
                    random_index = np.random.choice(number_of_goals, size=1, replace=False)

                    g = goals[random_index][0] #observation_dict['desired_goal'] #randomly choose from "goals"
...
#                     print('Training...')
            ...
            with open(f'./her_data_results/her_goid_seed_{self.seed}_env_freeAnt.csv', 'a') as fp:
                #columns: 0, 1, 2
            ...
            if True:
                ...
                #write succ to file
                with open(f'./her_data_results/experiment_1_seed_{self.seed}_env_freeAnt.csv','a') as fd:
                    fd.write(f'{evalu}\n')
