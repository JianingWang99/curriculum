...
from curriculum.experiments.her.her_evaluator import evaluate_states, rollout
...
def test_policy(policy, es, train_env, as_goals=True, visualize=True, sampling_res=1, n_traj=1, parallel=True,
                bounds=None, center=None):

    if parallel:
        return test_policy_parallel(policy, es, train_env, as_goals, visualize, sampling_res, n_traj=n_traj,
        ...
        for i in range(num_samples):
            for j in range(num_samples):
                ...
                for n in range(n_traj):
                    path = rollout(train_env, policy, es, animated=visualize, max_path_length=max_path_length, speedup=100)
                    paths.append(path)
...
def test_policy_parallel(policy, es, train_env, as_goals=True, visualize=True, sampling_res=1, n_traj=1,
    ...
    rewards, paths = evaluate_states(states, train_env, policy, es, max_path_length, as_goals=as_goals, n_traj=n_traj, full_path=True)
...
def test_and_plot_policy(policy, es, env, as_goals=True, visualize=True, sampling_res=1,
                         n_traj=1, max_reward=1, itr=0, time_spend = 0, report=None, center=None, limit=None, bounds=None):

    avg_totRewards, avg_success, states, spacing, avg_time = test_policy(policy, es, env, as_goals, visualize, center=center,
...
def plot_policy_means(policy, es, env, sampling_res=2, report=None, center=None, limit=None):  # only for start envs!
...
def main():
    ...
    es = OUStrategy(env_spec = train_env.spec)

    avg_totRewards, avg_success, goals, spacing = test_policy(policy, es, train_env, sampling_res=1)
