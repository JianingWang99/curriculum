...
class GoalExplorationEnv(GoalEnv, ProxyEnv, Serializable):
    ...
    def reset(self, reset_goal=True, reset_her=False, **kwargs):  # allows to pass init_state if needed
        if reset_goal:
            self.update_goal()
        #default behavior
        ret = ProxyEnv.reset(self, goal=self.current_goal, **kwargs)
        achieved_goal = self.transform_to_goal_space(ret)
...
#             self.update_goal(new_goal)
        if reset_her:
            return {'observation':ret, 'achieved_goal':achieved_goal, 'desired_goal':list(self.current_goal)}
        return ret

    def step(self, action):
        observation, reward, done, info = ProxyEnv.step(self, action)
        #observation shape here is 131
        info['achieved_goal'] = self.transform_to_goal_space(observation)
    ...
    def goal_distance(self, goal_a, goal_b=(0,4)):
    ...
    def compute_reward(self, achieved_goal, goal):
        ...
        return -(d > self.terminal_eps).astype(np.float32)
